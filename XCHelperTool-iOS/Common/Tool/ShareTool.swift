...
class ShareTool: NSObject {
   /// 分享
    ...
    private class func shareMessage(model: ShareModel) -> OSMessage {
        ...
        message.image = UIImagePNGRepresentation(model.logo)
...
class XCShareCodeController: UIViewController {
    ...
    private lazy var codeImageView: UIImageView = {
        ...
        imageView.image = QRCodeTool.qrCodeWithLogo(qrCodeData: model.url, logoImage: model.logo, logoScaleToSuperView: 0.2, isShowLogoJamb: true)
